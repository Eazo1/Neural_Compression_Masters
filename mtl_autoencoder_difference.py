...
#GAMMA = 0
ZETA = 1 # Hyperparameter for the Wasserstein-2 term
...
wandb.init(
    project="Covar_AE",
    config={
        "Zeta (Difference hyperparameter)": ZETA,
        #"embedding_dim": 64,
        #"num_embeddings": 256,
        "architecture": "AE",
        "dataset": "RGZ",
...
while iteration < num_training_updates:
    ...
    for images in train_loader:
        ...
        loss = mse_loss + ZETA * w_distance  # No .item() here!
        ...
        wandb.log({
                    ...
                    "train/loss_components.F*w_distance": (ZETA * w_distance.item())
                    })
        ...
        if iteration % 1000 == 0:
            ...
            with torch.no_grad():
                for val_images in valid_loader:
                    ...
                    loss_val = mse_loss_val + ZETA * w_distance_val
...
# Save the model
torch.save(autoencoder.state_dict(), "difference_autoencoder_model.pt")